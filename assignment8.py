...
def kozeny_carmen_plot(filename, **kwargs):   
    ...
    # fit
    ax.plot(x, fit[0] + fit[1] * x, label='Fit', color='red')

    # zero fit
    ax.plot(x, zero_fit[0] * x, label='Fit Through Zero', color='blue')
...
def contour_plot(filename, **kwargs):
    ...
    x = np.linspace(0, 130.75 * data.shape[1], num=data.shape[1])  # 54 grid blocks in the x direction, spaced 130.75 ft apart
    y = np.linspace(0, 130.75 * data.shape[0], num=data.shape[0])  # 44 grid blocks in the y direction, spaced 130.75 ft apart
    X, Y = np.meshgrid(x, y[::-1])    # Create 2D grid of x and y
